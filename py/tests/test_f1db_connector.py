...
def test_ensure_sqlite_path_follows_redirects_for_release_assets(tmp_path: Path, monkeypatch) -> None:
    connector = F1DBConnector(tmp_path)
    connector.client = _FakeClient(_build_archive_bytes())  # type: ignore[assignment]
    monkeypatch.setattr(
        connector,
    ...
    assert connector.client.calls == [  # type: ignore[attr-defined]
        {
            "method": "GET",
